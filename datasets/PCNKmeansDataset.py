...
class PCNKmeans(data.Dataset):
    # def __init__(self, data_root, subset, class_choice = None):
    def __init__(self, config):
        self.partial_points_path = config.PARTIAL_POINTS_PATH
        self.center_points_path = config.CENTER_POINTS_PATH
        ...
        for dc in self.dataset_categories:
            ...
            for s in samples:
                file_list.append({
                    ...
                    'center_path': [
                        self.center_points_path % (subset, dc['taxonomy_id'], s, i)
                        for i in range(n_renderings)
                    ],
                    'gt_path':
    ...
    def __getitem__(self, idx):
        ...
        for ri in ['partial', 'gt','center']:
            if ri == 'partial' or ri =='center':
                file_path = sample['%s_path' % ri]
                if type(file_path) == list:
                    file_path = file_path[rand_idx]
                data[ri] = np.loadtxt(file_path).astype(np.float32)
            elif ri == 'gt':
                file_path = sample['%s_path' % ri]
                if type(file_path) == list:
                    file_path = file_path[rand_idx]
        ...
        return sample['taxonomy_id'], sample['model_id'], (data['partial'], data['gt'], data['center'])
