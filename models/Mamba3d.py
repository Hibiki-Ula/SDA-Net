...
class Mamba3DBlock(nn.Module):
    def __init__(self,
        ...
        self.mixer2 = Mamba(num_query, num_query + 1, bimamba_type=bimamba_type)

        self.spnet = spconv.SparseSequential(
            spconv.SparseConv3d(dim, dim*2, 3, 2, indice_key="cp0"),
            spconv.SparseInverseConv3d(dim*2, dim, 3, indice_key="cp0"),  # need provide kernel size to create weight
        )
...
class Mamba3DEncoder(nn.Module):
    def __init__(self, num_query = 512,k_group_size=8, embed_dim=768, depth=4, drop_path_rate=0., num_group=128, num_heads=6, bimamba_type="v2",withoutlfa = False,order = 'z'):
        super().__init__()
        self.order = order
        self.num_group = num_group
        self.k_group_size = k_group_size
    ...
    def forward(self, center, x, pos):
        ...
        for _, block in enumerate(self.blocks):
              x = block(center, x + pos, self.order[i])
              i = i+1
        return x


class Block(nn.Module):
    def __init__(self, num_query=512,trans_dim=384, num_features=128, drop_path_rate=0, depth=6., k_group_size=4, bimamba_type="v4", withoutlfa = False,order = 'z'):
        super().__init__()
        ...
        self.blocks = Mamba3DEncoder(
            ...
            depth= depth,
            drop_path_rate= self.dpr,
            bimamba_type= bimamba_type,
            withoutlfa = withoutlfa,
            order = order
        )
    ...
    def forward(self, center, x):
        ...
        x = self.blocks(center, feature_x, pos)  # enter transformer blocks
...
class Mamba3d(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        if config.encoder_type == 'graph':
            self.grouper = DGCNN_Grouper(k = 8)
        else:
        ...
        self.withoutlfa = config.decoder_without_lfa

        self.encoder = Block(num_query=self.center_num[-1], trans_dim=self.trans_dim, num_features=self.grouper.num_features, drop_path_rate=self.drop_path_rate,
                             depth=self.depth[0], k_group_size=self.k_group_size, bimamba_type=self.bimamba_type, order = self.order)

        self.decoder = Block(num_query=self.num_query, trans_dim=self.trans_dim, num_features=self.encoder_dims, drop_path_rate=self.drop_path_rate,
                             depth=self.depth[1], k_group_size=self.k_group_size, bimamba_type=self.bimamba_type, withoutlfa = self.withoutlfa, order = self.order)
        
        self.encoder_t = Block(num_query=self.center_num[-1], trans_dim=self.grouper.num_features, num_features=self.trans_dim, drop_path_rate=self.drop_path_rate,
                             depth=self.depth[0], k_group_size=self.k_group_size, bimamba_type=self.bimamba_type, order = self.order)

        self.decoder_t = Block(num_query=self.num_query, trans_dim=self.encoder_dims, num_features=self.trans_dim, drop_path_rate=self.drop_path_rate,
                             depth=self.depth[1], k_group_size=self.k_group_size, bimamba_type=self.bimamba_type, withoutlfa = self.withoutlfa, order = self.order)
    ...
    def forward(self, xyz):
        bs = xyz.size(0)
        centerpt, feature = self.grouper(xyz, self.center_num) # b n c

        x = self.encoder(centerpt, feature)
        ...
        q = self.decoder(coarse_point_cloud, q)
        q = q[:,1:,]
        ...
        rebuild_points = rebuild_points.reshape(bs, -1, 3).contiguous()  # B N 3
