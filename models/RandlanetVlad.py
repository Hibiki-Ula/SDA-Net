...
class LocalFeatureAggregation(nn.Module):
    ...
    def forward(self, dspxyz, coords, features):
        r"""
            Forward pass
            Parameters
            ----------
        ...
        """
        knn_output = knn(dspxyz.cpu().contiguous(), coords.cpu().contiguous(), self.num_neighbors)
...
class RandLANet(nn.Module):
    def __init__(self, d_in, num_neighbors=8, device=torch.device('cpu')):
    ...
    def fps_downsample(xyz, num_group):
        fps_idx = pointnet2_utils.furthest_point_sample(xyz, num_group)
        dspxyz = pointnet2_utils.gather_operation(combined_x, fps_idx)
        return dspxyz

    def forward(self, input):
        N = input.size(1)  # B n 3 中的 n
        coords = input[..., :3].clone().cpu()
        dspxyz = self.fps_downsample(x, N//10)
        '''
                x = self.fc_start(input).transpose(-2,-1).unsqueeze(-1)  # b n 3 变 b n 8 1?
        ...
        for lfa in self.encoder:
            ...
            x = lfa(dspxyz, coords[:, :N//decimation_ratio], x)
...
if __name__ == '__main__':
    ...
    d_in = 7
    cloud = 5*torch.randn(1, 2048, d_in).to(device)
    model = RandLANet(d_in, 8, device)
...
if __name__ == '__main__':
    num_points = 4096
    sim_data = Variable(torch.rand(2, 1, num_points, 3))
