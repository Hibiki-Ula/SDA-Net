...
class SelfAttnBlockApi(nn.Module):
    ...
    def __init__(
        ...
        for block_token in block_tokens:
            assert block_token in ['attn', 'channel_attn', 'rw_deform', 'deform', 'graph', 'deform_graph'], f'got unexpect block_token {block_token} for Block component'
            if block_token == 'attn':
                self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
            elif block_token == 'channel_attn':
...
class LocalGrouper(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        if S == N:
            ...
            fps_idx = pointnet2_utils.furthest_point_sample(xyz, S).long()  # [B, npoint]
...
class ChainEncoder(nn.Module):
    def __init__(self, order='original',grid_size=0.02, embed_dims=128):
        super().__init__()
        self.embedding = ConvBNReLU1D(3, embed_dims, bias=False, activation="relu")
        self.order = order
        self.num_features = embed_dims
        ...
        self.reduce = [2048, 1024, 512, 256]
        self.k_stride = [1, 1, 1, 1]
        self.layers_count = [1, 2, 2, 4]
        for i in range(4):
            local_grouper = LocalGrouper(embed_dims, self.reduce[i], kneighbor = 12, use_xyz = True, normalize ="anchor",
                                         k_stride= self.k_stride[i])
            self.local_grouper_list.append(local_grouper)

            pre_block_module = PreExtraction(embed_dims, out_channels = embed_dims, blocks = 1,
                                             groups = 1,
                                             res_expansion = 1.0,
                                             bias = False, activation = 'relu', use_xyz = True)
            self.pre_blocks_list.append(pre_block_module)
        self.spnet = spconv.SparseSequential(
            spconv.SparseConv3d(embed_dims, embed_dims*2, 3, 2, indice_key="cp0"),
            spconv.SparseInverseConv3d(embed_dims*2, embed_dims, 3, indice_key="cp0"),  # need provide kernel size to create weight
        )
    ...
    def serialization(self, pos, feat, order="z",grid_size=0.02):
        ...
        return pos, feat, point_dict.batch, point_dict.grid_coord
    ...
    def forward(self, p):
        ...
        for i in range(4):
            ...
            for j in range(self.layers_count[i]):
                p, x, batch_idx, grid_coord= self.serialization(p, x, self.order[ser_count])
                bs, n, _ = x.shape
...
class PCTransformer(nn.Module):
    def __init__(self, config):
        ...
        assert self.encoder_type in ['graph', 'chain'], f'unexpected encoder_type {self.encoder_type}'
        ...
        # base encoder
        if self.encoder_type == 'chain':
            self.grouper = ChainEncoder(config.order, embed_dims=128)
...
class GlobalFea(nn.Module):
    ...
    #     pred_coarse, pred_split, pred_fine = ret
    ...
    #     # split loss
    #     _, _, num_sp, _ = pred_split.shape
    #     for i in range(num_sp):
    #         loss_split = self.loss_func(pred_split[:,:,i,:].squeeze(-2), gt)
    #         loss_fine = loss_fine + loss_split / self.num_query
