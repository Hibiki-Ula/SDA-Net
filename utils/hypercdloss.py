...
def get_loss(pcds_pred, partial, gt, sqrt=True):
    ...
    gt_2 = misc.fps(gt, P2.shape[1])
    gt_1 = misc.fps(gt_2, P1.shape[1])
    gt_c = misc.fps(gt_1, Pc.shape[1])
...
def get_loss_1(pcds_pred, partial, gt, sqrt=True):
    """loss function
    Args
        pcds_pred: List of predicted point clouds, order in [Pc, P1, P2, P3...]
    """
...
def get_loss1(pcds_pred, partial, gt, sqrt=True):
    ...
    # gt_c = misc.fps(gt, Pc.shape[1])

    # cdc = CD(Pc, gt_c)
    cdc = calc_cd_like_hyperV2(Pc, gt)

    # cd3 = CD(P3, gt)
    cde = calc_cd_like_hyperV2(Pe, gt)

    # partial_matching = PM(partial, P3)
    partial_matching = calc_cd_one_side_like_hyperV2(partial, Pe)

    loss_all = (cdc + cde + partial_matching) * 1e3
    losses = [cdc, cde, partial_matching]
    return cdc, cde, partial_matching
