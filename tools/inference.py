...
def inference_single(model, pc_path, args, config, root=None):
    ...
    '''
    if config.dataset.test._base_['NAME'] == 'KITTI':
        print(1)
    ...
    if config.dataset.test._base_['NAME'] == 'KITTI':
        ...
        dense_points = dense_points * m * 1.5
    ...
    if args.out_pc_root != '':
        ...
        np.savetxt(os.path.join(target_path, config.model.NAME + '.txt'), dense_points,fmt = "%.4f")
